...

{
  ...
  assert.ok(lifecycleSource.includes('CleanupServiceFiles(terre,state,servicePid,stoppingServiceId);await CleanupStaleServiceFiles(config,state)'),'lifecycle shutdown must remove owned discovery and then sweep stale service state');
