...
(function(root){
 ...
 function derive(path,source,parsed,types,animationData){
  ...
  for(const sentence of parsed.sentenceList||[]){
   ...
   const singleChoice=root.WebVideoTimelineCore.singleChooseInfo(row),sceneTarget=root.WebVideoTimelineCore.sceneChangeTarget(row),linearSceneChange=!!sceneTarget;row.singleLineHint=singleChoice.isHint;row.convertibleSingleChoose=singleChoice.convertible;row.linearSceneChange=linearSceneChange;row.ignoredInTiming=(controls.has(command)&&!singleChoice.isHint&&!linearSceneChange)||args.userForward===true||own(args,'when');if(row.ignoredInTiming){if(firstManualLine===null)firstManualLine=row.startLine;if(!row.convertibleSingleChoose)continue;}
   if(command==='bgm'&&firstBgmLine===null)firstBgmLine=row.startLine;
   if(hidden.has(command)&&!singleChoice.isHint&&!row.convertibleSingleChoose&&!linearSceneChange)continue;
   ...
   else if(linearSceneChange){row.kind='event';row.main=true;row.special=true;row.title='场景跳转 · '+sceneTarget;row.parts=[part('场景跳转',['目标：'+sceneTarget])];row.annotations.push('自动继续');}
