...
(function(root){
 ...
 function musicProject(value={}){
  ...
  if(!Array.isArray(value.tracks||[])||(value.tracks||[]).length>64)throw Error('音乐时间线最多包含 64 个片段');const tracks=(value.tracks||[]).map((track,i)=>{
   ...
   const normalized={lane:Math.floor(n('lane',0,0,63)),fullLength:!!track.fullLength,id:String(track.id||`track-${i}`),name:String(track.name||track.file?.split('/').pop()||'音乐'),file:safePath(track.file),startSeconds:n('startSeconds',0,0,86400),offsetSeconds:n('offsetSeconds',0,0,86400),durationSeconds,volume:n('volume',100,0,100),fadeInSeconds,fadeOutSeconds,loop:!!track.loop,enabled:track.enabled!==false};if(version===1&&track.scene)normalized.scene=safePath(track.scene);return normalized;
  });
