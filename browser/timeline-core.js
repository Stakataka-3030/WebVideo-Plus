...
(function(root){
  ...
  const ignoredInTiming=item=>ignoredCommands.has(item.command)||item.args.userForward===true||Object.prototype.hasOwnProperty.call(item.args,'when');
  function derive(path,source,parsed,types){
    ...
    for(const s of parsed.sentenceList||[]){
      ...
      item.ignoredInTiming=ignoredInTiming(item);if(item.ignoredInTiming){if(firstManualLine===null)firstManualLine=item.startLine;continue;}if(hiddenCommands.has(command))continue;
  ...
  root.WebVideoTimelineCore={derive,Selection,filter,fingerprint,ignoredInTiming};
