 ...
 for(let i=0;i<parsed.sentenceList.length;i++){
  ...
  if(cmd==='changeBg'||cmd==='changeFigure'){
   ...
   if(cmd==='changeFigure'){
    ...
    if(liveRuntime&&!activeLiveLifetimes.has(target))activeLiveLifetimes.set(target,{startMs:Math.round(cursor),endMs:null,target,source:visualName,position,bounds:nextBounds,line:range.start+1,motionEvents:[],expressionEvents:[],blinkEvents:[]});
    ...
    if(currentLifetime&&params.expression!==undefined)currentLifetime.expressionEvents.push({atMs:Math.round(cursor),expression:String(params.expression??''),line:range.start+1});
    if(currentLifetime&&params.blink!==undefined)currentLifetime.blinkEvents.push({atMs:Math.round(cursor),blink:String(params.blink??''),line:range.start+1});
