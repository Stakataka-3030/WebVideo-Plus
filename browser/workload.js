...
globalThis.__buildNativeWorkload=({script,parsed,media,animations,timing,root,project,sceneName,fps})=>{
 const events=[],audio=[],videoCues=[],muteWindows=[],singleLineHints=[],counts={},visualSources=new Map(),extraAnimations=new Map(),sourceLines=script.split(/\r?\n/);let cursor=0;
 ...
 const singleLineHintDuration=(cmd,params,content)=>{if(cmd!=='choose'||Number(params.defaultChoose)!==1||params.next===true)return 0;const ms=Number(params.wvpHint);if(!Number.isFinite(ms)||ms<100||ms>60000)return 0;const options=String(content||'').split(/(?<!\\)\|/);if(options.length!==1)return 0;const nodes=options[0].split(/(?<!\\):/);return nodes.length>1&&/^__wvp_hint_[A-Za-z0-9_]+$/.test(nodes[1].trim())?ms:0;};
 for(let i=0;i<parsed.sentenceList.length;i++){
  if(timing&&Number.isFinite(timing.lineTimes[i]))cursor=timing.lineTimes[i];const s=parsed.sentenceList[i];if(s.isLineBreakHolder)continue;const cmd=s.command===0?'say':s.commandRaw,params=Object.fromEntries(s.args.map(a=>[a.key,a.value])),raw=sourceLines[i]||'',hintMs=singleLineHintDuration(cmd,params,s.content);counts[cmd]=(counts[cmd]||0)+1;
  if(cmd==='comment'||!raw.trim()||raw.trim().startsWith(';'))continue;if(['changeScene','callScene','choose','jumpLabel','chooseLabel','if','getUserInput'].includes(cmd)&&!(cmd==='choose'&&hintMs))throw Error('无法导出第'+(i+1)+'行交互命令：'+cmd);if(cmd==='end')break;
  ...
  events.push({atMs:Math.round(cursor),line:i+1,command:cmd,script:snippet,voiceControlled:cmd==='say'&&!!params.vocal,mouthTarget:cmd==='say'?(speakerTarget(params)):null,loads:['changeBg','changeFigure','playVideo'].includes(cmd)});if(cmd==='playVideo')events.push({atMs:Math.round(cursor+videoDuration),line:i+1,command:'__finishVideo',script:'',loads:false});if(hintMs){const end=cursor+hintMs;singleLineHints.push({startMs:Math.round(cursor),endMs:Math.round(end),line:i+1});events.push({atMs:Math.round(end),line:i+1,command:'__singleLineEnd',script:'',loads:false});cursor=end;continue;}if(params.next)continue;
