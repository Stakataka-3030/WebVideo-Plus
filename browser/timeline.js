globalThis.__createNativeTimeline=async({script,policy})=>{
   ...
   const yieldQueue=[],yieldChannel=new MessageChannel();yieldChannel.port1.onmessage=()=>yieldQueue.shift()?.();const yieldTask=()=>new Promise(resolve=>{yieldQueue.push(resolve);yieldChannel.port2.postMessage(0);});const planStepMs=100,domWorkload=[];const sampleDom=()=>{if(typeof globalThis.__gpuDomCaptureNeeded!=='function')return;const capture=globalThis.__gpuDomCaptureNeeded();if(!capture)return;const state=globalThis.__gpuDomState,reason=state?.dirtyReason||'mutation',units=reason==='animation'||reason==='video'?Math.max(1,Math.round(planStepMs*(Number(policy.fps)||60)/1000)):1;domWorkload.push({atMs:origin===null?0:performance.now()-origin,reason,units});if(state)state.dirty=false;};const singleLineHintDuration=(s,params)=>{if(s.commandRaw!=='choose'||Number(params.defaultChoose)!==1||params.next===true)return 0;const options=String(s.content||'').split(/(?<!\\)\|/);if(options.length!==1)return 0;const nodes=options[0].split(/(?<!\\):/);if(nodes.length!==2||!/^__wvp_hint_[A-Za-z0-9_]+$/.test(nodes[1].trim()))return 0;const ms=Number(params.wvpHint);return Number.isFinite(ms)&&ms>=100&&ms<=60000?ms:1800;};const singleChooseItem=()=>{const main=document.getElementById('chooseContainer')?.firstElementChild;if(!main||main.children.length!==1)return null;const outer=main.firstElementChild;return outer&&outer.children.length===1?outer.firstElementChild:null;};const events=[],elastic=[],controlEvents=[];let origin=null,lastCommand='',lastParams={},lastLine=0,done=false,finishedAt=0,manualTimer=null,hintTimer=null,hintError='';
   // A timer queued during a non-blocking wait must not settle a later say.
   let autoGeneration=0;
   const cancelPendingAuto=()=>{autoGeneration++;if(core.gameplay.autoTimeout!=null){clearTimeout(core.gameplay.autoTimeout);core.gameplay.autoTimeout=null;}};
   globalThis.__nativeScheduleAuto=(fn,delay)=>{const generation=autoGeneration,startMs=performance.now()-origin,line=lastLine,eligible=lastCommand==='say'&&!lastParams.notend;return setTimeout(()=>{if(generation!==autoGeneration)return;if(pc.performList.some(p=>p.blockingAuto())){core.gameplay.autoTimeout=null;return;}if(eligible)elastic.push({startMs,endMs:performance.now()-origin,line});fn();},delay);};
   ...
   let steps=0;const maxSteps=108000;while(!done&&steps++<maxSteps){globalThis.__nativePlanProgress={phase:'advancing',line:lastLine+1,totalLines:core.sceneManager.sceneData.currentScene.sentenceList.length,videoSeconds:origin===null?0:(performance.now()-origin)/1000,events:events.length};await clock.runFor(planStepMs);await yieldTask();if(hintError)throw new Error(hintError);await __exportWaitForStageAssets();sampleDom();if(steps%50===0)await __reportNativePlan({...__nativePlanProgress,phase:'planning-progress',performs:pc.performList.map(p=>({name:p.performName,duration:p.duration,hold:p.isHoldOn}))});}
   core.events.textSettle.off(recordTextSettle);clearInterval(poll);clearTimeout(manualTimer);clearTimeout(hintTimer);w.nativeStopAuto();yieldChannel.port1.close();yieldChannel.port2.close();
