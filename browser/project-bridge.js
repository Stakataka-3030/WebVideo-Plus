...
const WebVideoProject=((terreApi)=>{
 ...
 const api={
  ...
  async activate(game){if(game&&(/[\\/%]/.test(game)||game==='.'||game==='..'))throw Error('项目路径无效');if(scope?.game===game)return;scope=game?{game,root:'games/'+game}:null;project=C.metadata();music={schemaVersion:2,enabled:false,players:1,tracks:[]};loadError='';const at=scope;loading=!!at;changed();if(!at)return;try{const m=await json(musicPath(at),{schemaVersion:2,enabled:false,players:1,tracks:[]});if(scope===at){music=C.musicProject(m);loading=false;changed();}}catch(e){if(scope===at){loadError=e.message;loading=false;changed();}}},
  ...
  async saveMusic(value){const next=C.musicProject(value);return withLock(async()=>{const at=scope,before=await read(musicPath(at),null),id=crypto.randomUUID(),file=projectPath('.webvideo-plus/backups/music-'+id+'.json',at),createdAt=new Date().toISOString();await write(file,JSON.stringify({schemaVersion:1,kind:'music',id,path:musicPath(at),createdAt,before:before||JSON.stringify({schemaVersion:2,enabled:false,players:1,tracks:[]}),after:next},null,2),{create:true});await write(musicPath(at),JSON.stringify(next,null,2),{create:true});if(scope===at){music=next;changed();}let warning='';try{const p=C.metadata(JSON.parse(JSON.stringify(project)));p.backups.unshift({id,kind:'music',label:'音乐时间线',path:musicPath(at),file,createdAt});p.backups=p.backups.slice(0,200);if(scope===at){project=p;changed();}}catch(e){warning='音乐已保存，但备份索引更新失败。备份文件：'+file+'。'+e.message;}return {...next,warning};});},
