...
> 0.4.10.2 发布候选已完成标准 Terre、MyGO 分发版和 Steam 版的本机安装/卸载回归；正式安装器待上传 GitHub Release。
...
发布安装器文件名为 `WebVideo+-Setup-0.4.10.2.exe`，面向已有 Terre 4.6.4 安装目录。当前发布候选已完成标准 Terre、MyGO 分发版和 Steam 版的本机安装/卸载回归；正式 Release 以最终普通构建产物为准。
