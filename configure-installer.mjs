...
replace('Size=new Size(690,208)','Size=new Size(690,430)');
s=s.replaceAll('advancedToggle.Checked?208:-208','advancedToggle.Checked?430:-430').replaceAll('advancedToggle.Checked?648:440','advancedToggle.Checked?890:460');
between('  foreach(Control field in advanced.Controls)field.Top+=80;','  start=new CheckBox',`  foreach(Control field in advanced.Controls)field.Top+=292;
  int moduleIndex=0;foreach(var id in ModuleCatalog.Advanced){var box=new CheckBox{Text=ModuleCatalog.Labels[id],ThreeState=true,Checked=true,AutoSize=true,Location=new Point(30+(moduleIndex%3)*210,3+(moduleIndex/3)*32)};moduleBoxes[id]=box;advanced.Controls.Add(box);moduleIndex++;}
  ...
  moduleNote=new Label{Location=new Point(30,204),Size=new Size(630,72),ForeColor=Color.DimGray};advanced.Controls.Add(moduleNote);foreach(var box in moduleBoxes.Values)box.CheckStateChanged+=(sender,eventArgs)=>{if(!syncModules)ReconcileModules();};ReconcileModules();
`);
...
s=s.replaceAll('Size=new Size(690,430),Visible=false','Size=new Size(690,430),AutoScroll=true,Visible=false');
