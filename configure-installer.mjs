...
s=s.replaceAll('WebGAL Video Exporter Setup','WebVideo+ Setup').replaceAll('WebGAL Video Exporter','WebVideo+').replaceAll('0.3.1.0','0.4.10.2').replaceAll('"webgal-native-exporter"','"webvideo-plus"');
...
replace('Size=new Size(690,128)','Size=new Size(690,208)');
...
replace('【内部版本 0.3.1 · C# / WebView2】','WebVideo+ 0.4.10 · 导出内核 0.4.10');
...
replace('Size=new Size(690,208)','Size=new Size(690,400)');
s=s.replaceAll('advancedToggle.Checked?208:-208','advancedToggle.Checked?400:-400').replaceAll('advancedToggle.Checked?648:440','advancedToggle.Checked?840:440');
between('  foreach(Control field in advanced.Controls)field.Top+=80;','  start=new CheckBox',`  foreach(Control field in advanced.Controls)field.Top+=272;
  int moduleIndex=0;foreach(var id in ModuleCatalog.Advanced){var box=new CheckBox{Text=ModuleCatalog.Labels[id],ThreeState=true,Checked=true,Location=new Point(30+(moduleIndex%3)*210,3+(moduleIndex/3)*32),Size=new Size(200,28)};moduleBoxes[id]=box;advanced.Controls.Add(box);moduleIndex++;}
  ...
  aiModule=new CheckBox{Text="安装基于生成式AI的组件 (Beta)",Checked=false,AutoSize=true,Location=new Point(245,173),Font=new Font(Font,FontStyle.Bold)};Controls.Add(aiModule);var aiInfo=new Label{Text="ⓘ",AutoSize=true,Font=new Font("Segoe UI Symbol",12),ForeColor=Color.FromArgb(55,115,170),Cursor=Cursors.Help,AccessibleName="生成式AI组件说明",AccessibleDescription="需要自行配置api key，测试版本。"};Controls.Add(aiInfo);aiInfo.Location=new Point(aiModule.Right+6,aiModule.Top-2);aiModule.SizeChanged+=(sender,eventArgs)=>aiInfo.Location=new Point(aiModule.Right+6,aiModule.Top-2);aiInfoTip.SetToolTip(aiInfo,"需要自行配置api key，测试版本。");FormClosed+=(sender,eventArgs)=>aiInfoTip.Dispose();aiModule.CheckedChanged+=(sender,eventArgs)=>{if(!syncModules)ReconcileModules();};
  moduleNote=new Label{Location=new Point(30,204),Size=new Size(630,52),ForeColor=Color.DimGray};advanced.Controls.Add(moduleNote);foreach(var box in moduleBoxes.Values)box.CheckStateChanged+=(sender,eventArgs)=>{if(!syncModules)ReconcileModules();};ReconcileModules();
`);
...
s=s.replaceAll('导出内核 0.4.10','导出内核 0.3.14-internal');
