 ...
 public static class SceneChain {
  ...
  public static async Task<object> BuildTimeline(string currentScene,string snapshotRoot,Func<string,string,Task<object>> parse){
   ...
   foreach(var name in names){
    string source=VideoWorkflow.CanonicalSource(File.ReadAllText(Files.Under(snapshotRoot,name)));var parsed=await parse(source,name);string next=StaticSuccessor(parsed);if(next!=null&&!names.Contains(next,StringComparer.OrdinalIgnoreCase))throw new FileNotFoundException("场景跳转目标不存在："+next);
    successors[name]=next;metadata[name]=J.O("scene",name,"hash",Files.HashText(source),"fingerprint",Fingerprint(source),"lines",source.Split('\n').Length,"nextScene",next??"");if(next!=null){List<string> list;if(!predecessors.TryGetValue(next,out list)){list=new List<string>();predecessors[next]=list;}list.Add(name);}
   }
