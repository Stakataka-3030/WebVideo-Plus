...
namespace NativeVideo {
 ...
 public static class Settings {
  public static Dictionary<string,object> Validate(object supplied){var d=J.O("width",1920,"height",1080,"fps",30,"mode","auto","bgmBaseMode","auto","workers",4,"textSpeed",50,"autoSpeed",50,"holdSeconds",1,"gpu","auto","engine","webgal","gpuRawMode","recommended","gpuRawPreferenceVersion",2,"gpuRawDom",true,"allowDecorativePixiCuts",false);foreach(var k in d.Keys.ToArray())if(J.Get(supplied,k)!=null)d[k]=J.Get(supplied,k);if(!new[]{"webgal","mygo"}.Contains(J.S(d,"engine")))throw new ArgumentException("导出引擎无效");int w=(int)J.N(d,"width"),h=(int)J.N(d,"height");if(!new[]{"1280x720","1920x1080","2560x1440","3840x2160"}.Contains(w+"x"+h)||!new[]{30d,60d}.Contains(J.N(d,"fps")))throw new ArgumentException("分辨率或帧率无效");double workers=J.N(d,"workers");if(workers<1||workers>32||workers!=Math.Truncate(workers))throw new ArgumentException("并行数需要填写 1–32 的整数");if(!new[]{"auto","manual","bgm"}.Contains(J.S(d,"mode"))||!new[]{"auto","manual"}.Contains(J.S(d,"bgmBaseMode"))||!new[]{"auto","high","low"}.Contains(J.S(d,"gpu"))||!new[]{"auto","off","recommended","quality","lossless","traditional","x264rgb","nvenc","amf","qsv","x264"}.Contains(J.S(d,"gpuRawMode")))throw new ArgumentException("导出模式、GPU 或编码管线选项无效");d["gpuRawMode"]=GpuEncoding.NormalizeMode(J.S(d,"gpuRawMode"));foreach(var key in new[]{"textSpeed","autoSpeed"})if(J.N(d,key)<-500||J.N(d,key)>100)throw new ArgumentException("播放速度需要在 -500–100 之间，推荐 0–100");if(J.N(d,"holdSeconds")<0||J.N(d,"holdSeconds")>60)throw new ArgumentException("等待时长无效");d["allowDecorativePixiCuts"]=J.B(d,"allowDecorativePixiCuts",false);return d;}
 }
