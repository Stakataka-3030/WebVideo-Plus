...
namespace NativeVideo {
 public static class GpuBenchmark {
  public static async Task Run(object request){
   string dir=J.S(request,"jobDir"),output=Files.Full(J.S(request,"output")),planning=Path.Combine(dir,"planning");Directory.CreateDirectory(dir);
   ...
   Files.EnsureParent(output);J.Write(output,result);Console.WriteLine(J.Text(result));
  }
 }
}
