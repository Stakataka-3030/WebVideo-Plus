...
namespace NativeVideo {
 public static class VideoWorkflow {
  ...
  public static Dictionary<string,object> Bounds(object timing,object range,int fps){
   int full=(int)Math.Round(J.N(timing,"durationSeconds")*fps);if(range==null)return J.O("startFrame",0,"endFrame",full,"totalFrames",full,"startSeconds",0,"durationSeconds",full/(double)fps);
   ...
   if(startMs==null)throw new ArgumentException("所选范围没有实际执行的语句");int first=(int)Math.Ceiling(startMs.Value*fps/1000-1e-7),last=endMs.HasValue?(int)Math.Ceiling(endMs.Value*fps/1000-1e-7):full;last=Math.Min(last,full);
  ...
  public static async Task<object> SnapshotMusic(string project,string jobDir,bool enabled,string scene=""){
   if(!enabled)return null;string file=Path.Combine(project,"video-project.json");if(!File.Exists(file))return null;var data=J.Read(file);if(!J.B(data,"enabled",false))return null;if(J.N(data,"schemaVersion",1)!=1)throw new ArgumentException("成片音乐文件版本不受支持");
   var result=new List<object>();var occupied=new Dictionary<int,List<Tuple<double,double>>>();var tracks=J.A(J.Get(data,"tracks"));if(tracks.Count>64)throw new ArgumentException("单个音乐时间线最多包含 64 个片段");
   foreach(var track in tracks){if(!J.B(track,"enabled",true)||scene!=""&&J.S(track,"scene")!=""&&J.S(track,"scene").Replace('\\','/')!=scene.Replace('\\','/'))continue;string relative=J.S(track,"file");if(Path.IsPathRooted(relative)||relative.Replace('\\','/').Split('/').Any(s=>s==".."))throw new ArgumentException("音乐必须位于当前项目内");string source=Files.Under(project,relative);if(!File.Exists(source))throw new FileNotFoundException("找不到成片音乐："+relative);if(new FileInfo(source).Length>128L*1024*1024)throw new ArgumentException("单个成片音乐文件超过 128 MB");
    ...
    double sourceDuration=await Commands.Duration(source)/1000,offset=Number(track,"offsetSeconds",0,0,86400),length=Number(track,"durationSeconds",sourceDuration,.001,86400),start=Number(track,"startSeconds",0,0,86400),fadeIn=Number(track,"fadeInSeconds",0,0,length),fadeOut=Number(track,"fadeOutSeconds",0,0,length),volume=Number(track,"volume",100,0,100);bool loop=J.B(track,"loop");if(J.B(track,"fullLength")){offset=0;length=sourceDuration;loop=false;fadeIn=0;fadeOut=0;int lane=(int)Number(track,"lane",0,0,63);if(!occupied.ContainsKey(lane))occupied[lane]=new List<Tuple<double,double>>();if(occupied[lane].Any(interval=>start<interval.Item2-.001&&start+length>interval.Item1+.001))throw new ArgumentException("同一播放器行中的音乐不能重叠，请移动音乐或增加播放器行");occupied[lane].Add(Tuple.Create(start,start+length));}
    ...
    string copy=Path.Combine(jobDir,"music-snapshot",Guid.NewGuid().ToString("N")+ext);Files.CopyFile(source,copy);result.Add(J.O("file",copy,"originalFile",relative,"scene",J.S(track,"scene").Replace('\\','/'),"startSeconds",start,"offsetSeconds",offset,"durationSeconds",length,"volume",volume,"fadeInSeconds",fadeIn,"fadeOutSeconds",fadeOut,"loop",loop));
   }return J.O("replaceGameBgm",result.Count>0&&J.B(data,"replaceGameBgm"),"tracks",result);
  }
  public static void AddMusic(object plan,object request){
   var music=J.Get(request,"musicTimeline");if(music==null)return;var audio=J.A(J.Get(plan,"audio"));if(J.B(music,"replaceGameBgm"))audio=audio.Where(a=>J.S(a,"kind")!="bgm"||J.S(a,"origin")=="playlist").ToList();string scene=J.S(request,"scene").Replace('\\','/');
   foreach(var t in J.A(J.Get(music,"tracks"))){if(J.S(t,"scene")!=""&&J.S(t,"scene")!=scene)continue;double start=J.N(t,"startSeconds")*1000,length=J.N(t,"durationSeconds")*1000;audio.Add(J.O("kind","video-music","path",J.S(t,"file"),"atMs",start,"endMs",start+length,"sourceOffsetMs",J.N(t,"offsetSeconds")*1000,"volume",J.N(t,"volume")/100,"fadeMs",J.N(t,"fadeInSeconds")*1000,"fadeOutMs",J.N(t,"fadeOutSeconds")*1000,"loop",J.B(t,"loop")));}
