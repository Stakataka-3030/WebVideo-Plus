...
## 1.0.4 / 安装器 Win32 版本 1.0.4.0 / 内部版本 0.8.3

- 安装器高级设置新增可选的 FFmpeg 目录。可选择同时包含 `ffmpeg.exe`、`ffprobe.exe` 的目录，或含 `bin` 子目录的上一级目录；验证两个程序的版本输出后，使用该目录完成安装并保存在当前 Terre 实例配置中，不修改系统 PATH，也不删除用户所选文件。
- FFmpeg 下载失败时可直接改选本地目录；手动目录无法读取或验证失败时可改用自动检测 / 下载。下载缓存继续保留以供重试，WebView2 的下载授权与检测保持不变。响应式高级设置同步容纳新字段和说明文字。
- 产品版本升至 1.0.4；导出内核保持 0.6.43。详见 [1.0.4 发行说明](RELEASE_NOTES_1.0.4.md)。

## 1.0.3 / 安装器 Win32 版本 1.0.3.0 / 内部版本 0.8.2 / 导出内核 0.6.43

- 外部 WebGAL 运行时只有在导出器所需的预览命令、剧本解析器和舞台探针均已存在时才用于导出；缺少完整探针的工程或 Terre 模板运行时会回退内置 WebGAL 4.6.4。此前仅从内置 bundle 复制了 `core/store` 的第一条赋值，漏掉 `parseScene` 与 `stageManager`，可能在规划阶段等待引擎就绪直至超时。
- 引擎启动超时时，日志现在分别记录解析器、预览命令、舞台和 Pixi 应用的就绪状态，以及运行时来源、回退原因与本地 HTTP 4xx 资源线索。HTTP 4xx 仅供诊断，不直接视为故障原因。
- 多 Worker 前文恢复改用 WebGAL 解析后的有效语句索引，而不是拼接临时剧本的物理换行数；这避免最后一行是多行语句续行时，WebGAL 已完成快进却被导出器误判为未到目标而等待超时。恢复超时会记录当前场景、语句索引和快进状态，并更新流水线修订以避免沿用旧分片缓存。
- 产品版本从 1.0.2 升为 1.0.3，详见 [1.0.3 发行说明](RELEASE_NOTES_1.0.3.md)。

## 1.0.2 / 安装器 Win32 版本 1.0.2.0

- 修复勾选生成式 AI 组件时，Manager 向 Terre 的 `video-export.next-*` 暂存目录复制 `ai-runtime`，遇到过深的 TypeScript 类型声明路径导致安装失败的问题。仅跳过 `dist-types`、`.d.ts` 和 source map 等非运行文件；运行入口与必需 JS 复制失败仍会报错，其他模块继续按原有严格复制逻辑处理。
- 产品版本从 1.0.1 升为 1.0.2；导出内核仍为 0.6.42。详见 [1.0.2 发行说明](RELEASE_NOTES_1.0.2.md)。

## 1.0.1 / 安装器 Win32 版本 1.0.1.0

- 正式产品版本从 1.0.0 更新到 1.0.1；内部开发标识仍为 0.8.1，导出内核仍为 0.6.42。
- 汇总 1.0.0 发布后的 Cubism 3/4 分段恢复、多行对话逐字字幕及安装器错误提示修复，详见 [1.0.1 发行说明](RELEASE_NOTES_1.0.1.md)。
- 多动作组的显式动作在不同 Worker 中使用一致索引；异步启动后的动作队列起点对齐剧本事件时间。恢复时若启动调用返回假值，但目标动作确已入队，仍按队列状态完成时间对齐。
- Haru 多动作场景的单/双进程切点逐帧对照，以及一个 Cubism 4 模型的待机、眨眼、非循环动作场景验证通过；该验证不覆盖所有模型与特殊配置。
...
- 修复 GPU DOM 文字图集里的逐字字幕位置：移动字符到图集时保留原始小数像素相位，贴回原画面时只平移整数部分，避免两行不同基线的小数被重复量化而呈现相反方向的细小错位。
- 更新导出缓存修订，旧任务重试会重新规划和渲染，避免沿用带偏移的缓存片段。产品版本与安装器文件版本仍保持 1.0.0。


### 内部版本 0.7.52 / 导出内核 0.6.40

- 补齐 0.7.51 分层导出模块化改动的第二处安装阶段门控列表，确保实际 staging 不会尝试读取不存在的 `features/modules/layerExport.js`。
- 更新分层导出回归断言：顶栏三个分层入口现在必须由 `layerExport` 控制，同时校验“导入与导出”菜单存在对应模块门控，避免以后再次退回到 `exporter`。
- 本次不修改分层导出运行实现与导出内核，仅完成安装器模块化接线的构建闭环。

### 内部版本 0.7.51 / 导出内核 0.6.40

- 将“分层导出”补入安装器模块目录，新增独立可选模块 `layerExport`，界面显示为“分层导出（舞台/对话框/音轨）”。
- `layerExport` 依赖“导出视频”模块；安装器会自动补齐并灰选其依赖，不复制第二套导出实现，继续复用现有导出面板、`LayerExport.cs` 与导出内核。
- WebVideo+ 顶栏和导入与导出菜单改为仅在安装 `layerExport` 时显示“导出舞台 / 导出对话框 / 仅音轨”；普通“导出视频”仍由 `exporter` 单独控制。
- 升级旧安装时，如果原先已安装“导出视频”，会自动补入 `layerExport`，避免升级后原本可见的分层导出入口突然消失。
- 安装阶段把 `layerExport` 作为功能门控模块处理，不要求不存在的独立前端 JS 文件；后端实现继续随导出内核提供。
...
- 对齐 Cubism2 自带呼吸的模型年龄，覆盖迟上场、没有显式 motion 或标准 idle 的人物，不为静止人物强行启动动作。
- 非循环 motion 已结束且无 idle 时，通过原生队列和独立参数代理重建结束后的保存参数，不重新播放该动作；只计算动作参数，不增加整段 GPU 画面 replay，保留现有3秒物理预热和并行切段。
- 增加 naturalAgeAligned / completed-motion 诊断，并使旧分片缓存失效。


### 内部版本 0.7.48 / 导出内核 0.6.39

- 修复 0.7.45 安装器 stdout/stderr 管道收尾补丁在旧 .NET Framework C# 编译器上的构建失败。此前使用 `var outputClosed=...,errorClosed=...` 在一个隐式类型局部变量声明中包含两个声明符，会触发 `CS0819: 隐式类型的局部变量不能有多个声明符`。
- 将两个 `ManualResetEventSlim` 局部变量拆成独立声明，运行时语义不变；stdout/stderr EOF 的 1 秒有界等待和取消异步读取逻辑继续保留。
- 回归测试新增 C# 5 兼容断言，明确禁止恢复该多声明符 `var` 写法。本次仅修安装器构建，导出内核保持 `0.6.39`，0.7.47 Live2D 诊断与 3 秒 bounded physics warmup 不变。
...
- 安装器现在仍严格等待直接子进程退出并保留退出码、安装日志与 `[FORCE_AVAILABLE]` 错误捕获，但 stdout/stderr 的退出后收尾各最多等待 1 秒；若长寿命后代仍持有管道，就取消对应异步读取，不再让安装器无限等待。
- 新增安装器回归断言，要求异步 stdout/stderr 收尾有明确上限，并禁止恢复退出后的无界 `WaitForExit()`。
