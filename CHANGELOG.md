...
## 1.0.1 / 安装器内部版本 1.0.1.0

**音乐时长匹配提速。** “调整故事速度匹配音乐”不再最多反复校准 6 次并额外做边界核实，匹配阶段现在最多进行 2 次实际校准。第二次会优先根据已有基准与首轮结果做线性插值，直接逼近目标速度；达到可接受误差后立即停止。

匹配容差按音乐长度放宽到约 0.5–2 秒，符合成片场景对秒级误差的实际需求。两轮后若仍未落入容差，不再继续追求高精度收敛，而是采用已经测得的误差最小结果；只有实际测到速度边界仍无法覆盖目标时才报告 BGM 过长或过短。导出内核保持 `0.5.3`。

## 1.0.0 / 安装器内部版本 1.0.0.0

**首个正式发布版。** WebVideo+ 产品版本进入 `1.0.0`，安装器 Win32 文件版本使用 `1.0.0.0`；导出内核继续保持 `0.5.3`，本次版本切换不改变现有导出编码与渲染路径。

安装器改为可调整窗口大小，并增加最小尺寸、状态文本空间、模块说明空间和高级区域滚动容错，降低 Windows 高 DPI / 显示缩放下文字被裁切的概率。安装器启动时会优先复用历史/默认 Terre 路径；无有效记录时快速检查桌面与开始菜单快捷方式，并在常见安装位置浅层搜索 `WebGAL_Terre.exe` / `WebGAL Terre.exe`。若仍未找到，会明确提示用户手动选择；手动入口同时接受 Terre 文件夹、主程序 EXE 和 `.lnk` 快捷方式。稳定版安装器文件名使用面向用户的产品版本（`WebVideo+-Setup-1.0.0.exe`）。
