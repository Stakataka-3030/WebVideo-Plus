...
- 多 Worker 分段改为生命周期感知的重放锚点：切点若穿过动画、视频、intro、Pixi 演出或持续模型状态，会自动向前回退到安全重放点；无法安全中途恢复的持续运行时效果会从故事起点重放。重放成本过高时自动降低实际 Worker 数，而不是牺牲成片一致性。
