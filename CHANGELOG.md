...
成功导出与时间分析完成后会自动删除重型工作缓存；失败或取消任务保留分片和素材快照以支持继续重试。规划进程和每个渲染 worker 的 WebView2 profile 正常结束即清理；进程被强制终止时，服务会在任务退出或下次启动时补扫残留 profile。升级后服务启动也会尝试清理历史“已完成”任务遗留的旧重型缓存。
