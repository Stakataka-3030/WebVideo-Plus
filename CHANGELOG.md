...
单 worker 实测进一步确认 WebGAL/Pixi 原生 framebuffer 为固定 2560×1440 舞台，而导出视口可为 1920×1080。对比发现 WebGL2 framebuffer blit 到 1920×1080 后再 readPixels 反而比直接读取 2560×1440 默认 backbuffer 更慢，因此 SharedBuffer benchmark 默认回到 direct 原生 readback，并保留 `scale` 作为诊断对照。新增 `renderer` 模式：直接将 Pixi renderer resize 到输出尺寸，并对 2560×1440 逻辑舞台施加等比例全局 stage 缩放，测试能否省掉 1440p backing buffer 和二次 blit。新增 `--gpu-readback-host-copy true`，逐帧将完整 SharedBuffer 通过 OpenStream 读入宿主复用缓冲区，以测量正式编码管线中的 host 消费成本。
