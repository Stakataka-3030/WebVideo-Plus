...
- 修复 `-notend -next` / `wait` 链后旧对白的 `textSettle` 仍可能在新对白已经开始后触发的问题。导出器现在给每个 say perform 绑定当前 `currentDialogKey`；旧对白结束时只有 owner 仍等于当前对白才允许把文字标记为 settled，避免上一句把下一句瞬间全部显示。
