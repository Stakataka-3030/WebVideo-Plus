...
0.4.10.2 发布候选已完成标准 Terre、MyGO 分发版和 Steam 版的本机安装/卸载回归。正式 Release 以最终普通构建产物为准。
...
发布构建流程现在会清理旧的 `package/`、`.build/` 和正式解包输出，避免删除过的旧文件残留进安装器。最终用户发布物只需要 `WebVideo+-Setup-0.4.10.2.exe`；安装器无需同名 `.exe.config` sidecar。
