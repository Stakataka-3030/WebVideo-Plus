export function applyInstallerEnhancements(source,{productVersion,internalVersion,kernelVersion}){
 ...
 between(' public void Install(', '\n}\npublic class InstallationState', ` public void Install(RuntimePlan p,string terre,string games,string output,string url,string dataDir,string workDir,string installCache,bool keepRecovery,bool force,bool start){
  ...
  args.AddRange(new[]{"--ffmpeg-source-dir",p.FFmpegSourceDir??""});Run(Path.Combine(p.Payload,"WebVideoPlus.Manager.exe"),args,p.RuntimePath,p.Payload);if(start&&p.Modules.Length>0){Report("正在启动 Terre 与 WebVideo+…");Run(Path.Combine(p.Payload,"WebVideoPlus.Manager.exe"),new[]{"launch","--terre-dir",terre},p.RuntimePath,p.Payload);}Report("安装完成。",100);
 }
...
`);
 ...
 replace('TextBox terre,games,output,url;','TextBox terre,games,output,url,dataDir,workDir,installCache,ffmpegDir;');
 ...
 replace(
  ...
  ffmpegDir=Field(advanced,"FFmpeg 目录（可选）","",512,true);
  advanced.Controls.Add(new Label{Text="选择含 ffmpeg.exe 和 ffprobe.exe 的文件夹，或其上一级（内有 bin）。留空则自动检测 / 下载；手动目录以后请勿移动。",Location=new Point(170,546),Size=new Size(540,42),ForeColor=Color.DimGray});
  keepRecovery=new CheckBox{Text="保留 Terre 原始文件的长期恢复备份（推荐）",Checked=true,AutoSize=true,Location=new Point(170,602)};advanced.Controls.Add(keepRecovery);advanced.Controls.Add(new Label{Text="关闭后仍会为本次安装 / 更新 / 拆卸创建临时回滚副本，操作成功后立即删除。",Location=new Point(170,630),Size=new Size(540,42),ForeColor=Color.DimGray});advanced.AutoScrollMinSize=new Size(0,690);`
 );
 ...
 async Task<bool> PrepareRuntimesWithFFmpegFallback(RuntimePlan current,string selectedDirectory){
  if(!ModuleCatalog.NeedsKernel(current.Modules))return true;
  if(!String.IsNullOrWhiteSpace(selectedDirectory)){
   try{await Task.Run(()=>engine.UseLocalFFmpeg(current,selectedDirectory));ffmpegDir.Text=current.FFmpegSourceDir;}
   catch(Exception manualError){string alternative=current.NeedFFmpeg?"自动下载":"已自动检测到的 FFmpeg";var choice=MessageBox.Show(this,"所选本地 FFmpeg 无法使用："+manualError.Message+Environment.NewLine+Environment.NewLine+"改用"+alternative+"继续吗？","本地 FFmpeg 不可用",MessageBoxButtons.YesNo,MessageBoxIcon.Warning,MessageBoxDefaultButton.Button2);if(choice!=DialogResult.Yes){status.Text="请在高级设置重新选择 FFmpeg 目录，或清空该项后重试。";return false;}ffmpegDir.Text="";}
  }
  bool consent=current.Downloads.Count==0;
  if(!consent){var text="需要下载并安装以下组件："+Environment.NewLine+Environment.NewLine+string.Join(Environment.NewLine,current.Downloads.Select(x=>"• "+x))+Environment.NewLine+Environment.NewLine+"安装缓存目录："+Environment.NewLine+engine.Root+Environment.NewLine+Environment.NewLine+"不会更改系统 PATH。是否同意下载并继续安装？";consent=MessageBox.Show(this,text,"允许安装缺失的运行组件？",MessageBoxButtons.YesNo,MessageBoxIcon.Question,MessageBoxDefaultButton.Button2)==DialogResult.Yes;}
  if(!consent){status.Text="已取消，未下载或安装依赖。";return false;}
  for(;;){
   Exception downloadError=null;try{await Task.Run(()=>engine.EnsureRuntimes(current,true));return true;}catch(Exception error){downloadError=error;}
   if(!current.NeedFFmpeg)throw downloadError;
   var choice=MessageBox.Show(this,"FFmpeg 自动下载未完成："+downloadError.Message+Environment.NewLine+Environment.NewLine+"要改选已解压的本地 FFmpeg 文件夹继续吗？下载进度会保留。","FFmpeg 下载失败",MessageBoxButtons.YesNo,MessageBoxIcon.Warning,MessageBoxDefaultButton.Button2);
   if(choice!=DialogResult.Yes)throw downloadError;
   using(var dialog=new FolderBrowserDialog{Description="选择包含 ffmpeg.exe 与 ffprobe.exe 的文件夹，或其上一级文件夹",SelectedPath=Directory.Exists(ffmpegDir.Text)?ffmpegDir.Text:Environment.GetFolderPath(Environment.SpecialFolder.UserProfile),ShowNewFolderButton=false}){
    if(dialog.ShowDialog(this)!=DialogResult.OK){status.Text="未选择本地 FFmpeg；下载进度已保留，可在高级设置指定目录后重试。";return false;}
    try{await Task.Run(()=>engine.UseLocalFFmpeg(current,dialog.SelectedPath));ffmpegDir.Text=current.FFmpegSourceDir;}
    catch(Exception manualError){var retry=MessageBox.Show(this,"所选目录不可用："+manualError.Message+Environment.NewLine+Environment.NewLine+"改为重试自动下载？","本地 FFmpeg 不可用",MessageBoxButtons.YesNo,MessageBoxIcon.Warning,MessageBoxDefaultButton.Button2);if(retry!=DialogResult.Yes){status.Text="请在高级设置重新选择 FFmpeg 目录后重试。";return false;}ffmpegDir.Text="";}
   }
  }
 }
...
`);
 ...
 between(' async Task Install(){','\n async Task Uninstall(){',` async Task Install(){
  ...
  engine=new SetupEngine(ic,Report);SetBusy(true);bool forceInstall=false;
  try{
   plan=await Task.Run(()=>engine.Inspect(engine.ExtractPayload(),selected));if(!await PrepareRuntimesWithFFmpegFallback(plan,ffmpegDir.Text))return;
