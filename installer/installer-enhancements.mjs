...
`);
 ...
 between(' async Task Install(){','\n async Task Uninstall(){',` async Task Install(){
  ...
  try{ValidateStoragePath(d,"WebVideo+ 数据目录",t,g);ValidateStoragePath(w,"导出工作缓存",t,g);ValidateStoragePath(ic,"安装缓存目录",t,g);if(PathsOverlap(d,w)||PathsOverlap(d,ic)||PathsOverlap(w,ic))throw new Exception("WebVideo+ 数据目录、导出工作缓存和安装缓存目录不能互相包含或使用同一路径。");}catch(Exception pathError){MessageBox.Show(this,pathError.Message,"请核对存储路径");return;}
  ...
  try{
   ...
   catch(Exception first){if(!first.Message.Contains("[FORCE_AVAILABLE]"))throw;var choice=MessageBox.Show(this,first.Message.Replace("[FORCE_AVAILABLE]","").Trim()+"\\n\\n是否强制修复？\\n安装器会先创建事务性临时回滚副本，操作成功后立即删除。强制修复会尽量完成变更，但无法安全合并的外部修改可能被恢复为已知原版基线。","检测到安装状态不一致",MessageBoxButtons.YesNo,MessageBoxIcon.Warning,MessageBoxDefaultButton.Button2);if(choice!=DialogResult.Yes)throw;await Task.Run(()=>engine.Install(plan,t,g,o,u,d,w,ic,keep,true,launch));}
 ...
 between(' async Task Uninstall(){','\n}\npublic static class InstallerMain',` bool ShowUninstallOptions(out bool deleteCache,out bool deleteData){bool selectedCache=true,selectedData=false;using(var f=new Form{Text="卸载 WebVideo+",ClientSize=new Size(610,330),FormBorderStyle=FormBorderStyle.FixedDialog,MaximizeBox=false,MinimizeBox=false,StartPosition=FormStartPosition.CenterParent,Font=Font,BackColor=BackColor}){
  ...
  f.Controls.Add(new Label{Text="包括设置、任务历史、日志、自动备份、滤镜 / 角色映射 / 预制效果 / AI 配置，以及长期恢复备份和旧版全局配置。",Location=new Point(52,202),Size=new Size(520,48),ForeColor=Color.DimGray});
 ...
 async Task Uninstall(){bool deleteCache,deleteData;if(!ShowUninstallOptions(out deleteCache,out deleteData))return;string target=terre.Text,d=Path.GetFullPath(dataDir.Text),ic=Path.GetFullPath(installCache.Text);engine=new SetupEngine(ic,Report);SetBusy(true);try{
   ...
   try{await Task.Run(()=>runForce(false));}catch(Exception first){if(!first.Message.Contains("[FORCE_AVAILABLE]"))throw;var choice=MessageBox.Show(this,first.Message.Replace("[FORCE_AVAILABLE]","").Trim()+"\\n\\n是否强制拆卸？\\n安装器会先创建事务性临时回滚副本并尽量恢复 Terre；强制拆卸可能用已知原版基线替换不一致文件。","检测到安装状态不一致",MessageBoxButtons.YesNo,MessageBoxIcon.Warning,MessageBoxDefaultButton.Button2);if(choice!=DialogResult.Yes)throw;await Task.Run(()=>runForce(true));}
   engine.CleanupInstallCache(deleteCache,deleteData);string defaultInstallCache=Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),"WebGALVideoExporter");if(!String.Equals(Path.GetFullPath(ic).TrimEnd('\\','/'),Path.GetFullPath(defaultInstallCache).TrimEnd('\\','/'),StringComparison.OrdinalIgnoreCase)){var defaultEngine=new SetupEngine(defaultInstallCache,Report);defaultEngine.CleanupInstallCache(deleteCache,deleteData);}loadedModulesPath="";status.Text=deleteCache&&deleteData?"WebVideo+ 已完整卸载，缓存和用户数据已清理。":"WebVideo+ 已卸载。已导出的 MP4 和 WebGAL 游戏工程保留。";RefreshInstallation(false);MessageBox.Show(this,status.Text,"卸载完成");
