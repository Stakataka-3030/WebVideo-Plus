...
public class SetupEngine {
 ...
 public void Run(string exe,IEnumerable<string> arguments,string runtimePath,string cwd){
  Check();var errors=new List<string>();using(var p=new Process()){p.StartInfo=new ProcessStartInfo(exe,string.Join(" ",arguments.Select(Quote))){UseShellExecute=false,CreateNoWindow=true,WorkingDirectory=cwd,RedirectStandardOutput=true,RedirectStandardError=true};p.StartInfo.EnvironmentVariables["PATH"]=runtimePath+Path.PathSeparator+(Environment.GetEnvironmentVariable("PATH")??"");
   p.OutputDataReceived+=(s,e)=>{if(e.Data!=null)AppendProcessLog(e.Data);};p.ErrorDataReceived+=(s,e)=>{if(e.Data!=null){lock(errors)errors.Add(e.Data);AppendProcessLog(e.Data);}};p.Start();p.BeginOutputReadLine();p.BeginErrorReadLine();var clock=Stopwatch.StartNew();
   while(!p.WaitForExit(500)){if(Canceled||clock.Elapsed.TotalMinutes>15){using(var kill=Process.Start(new ProcessStartInfo("taskkill.exe","/PID "+p.Id+" /T /F"){UseShellExecute=false,CreateNoWindow=true}))kill.WaitForExit(5000);Check();throw new Exception("安装步骤超时，请查看安装日志");}}p.WaitForExit();if(p.ExitCode!=0){string forceError;lock(errors)forceError=errors.FirstOrDefault(line=>line.Contains("[FORCE_AVAILABLE]"));if(forceError!=null)throw new Exception(forceError);throw new Exception("安装步骤未完成（退出码 "+p.ExitCode+"），详细原因已写入安装日志。");}}
 }
